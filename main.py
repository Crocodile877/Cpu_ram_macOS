...
import sys
from operations import AllProcessors
from update import Configure_widgets

class Annex(tk.Tk, Configure_widgets):
...
# разобраться с захватом мыши___________________________________________++++++++++
#          self.bind_all('<Enter>', self.enter_mouse)
#          self.bind_all('<Leave>', self.leave_mouse)
    ...
    def make_bar_cpu_usage(self):
        ...
        self.ram_lab = ttk.Label(self.bar,text = 'text',anchor = tk.CENTER)
...
# не работают 2 кнопки в минимальном режиме
    ...
    def enter_mouse(self, event):
        if self.combo_win.current() == 1 or 0:
            self.geometry()
